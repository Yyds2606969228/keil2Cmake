...
C_CONSTRAINTS_ZH: dict[str, str] = {
    ...
    "RNN": "当前子集：float32，forward，num_directions=1。",
    "GRU": "当前子集：float32，forward，num_directions=1，linear_before_reset=0。",
    "LSTM": "当前子集：float32，forward，num_directions=1，不支持 peephole。",
...
FAMILY_LABELS = {
    ...
    "recurrent": {"zh": "循环网络", "en": "Recurrent"},
    "vision": {"zh": "视觉检测", "en": "Vision & Detection"},
...
I18N = {
    "zh": {
        ...
        "counterpart": "- English version: `onnx_opset12_coverage_matrix_EN.md`",
        "generated": "生成时间（UTC）",
        "scope": "统计范围",
        ...
        "default_cli": "默认命令",
        "quant_rule": "量化判断",
        "quant_rule_val": "基于模型 Q/DQ 与张量 dtype 自动推断",
        "weights_rule": "权重存储",
        ...
        "metric_c": "C 后端已覆盖",
        "metric_quant": "量化覆盖（`quant_ops ∩ C`）",
        "metric_full": "完整支持（无额外约束）",
        "metric_cons": "受约束支持",
        "metric_missing": "未实现",
        "quick_line_1": "- C 后端覆盖：`{c_supported}/{total}`（{c_rate:.1f}%）",
        "quick_line_2": "- 量化覆盖（按 `quant_ops` 口径）：`{quant_cov}/{c_supported}`（{quant_rate:.1f}%）",
        "quick_line_3": "- 受约束算子：`{cons_count}` 个；建议优先关注该清单",
        "legend_full": "- `完整支持`：已实现且无额外子集约束",
        "legend_cons": "- `受约束`：已实现，但存在输入形状/类型/属性等约束",
        "legend_missing": "- `未实现`：当前 C 后端未提供实现",
        "none": "_无_",
        "section_a": "### A. 受约束算子",
        "section_b": "### B. 完整支持算子",
        "section_c": "### C. 未实现算子",
    ...
    "en": {
        ...
        "counterpart": "- 中文版本: `onnx_opset12_coverage_matrix.md`",
