...
- 英文版: `onnx_opset12_coverage_matrix_EN.md`
- 生成时间（UTC）: `2026-02-25 03:27:29Z`
- 统计范围: `domain in ('', 'ai.onnx')` 且 `since_version <= 12`
- 后端规则: 仅统计 C 后端
- 默认 CLI: `--weights flash --emit c`
- 量化判定: 基于模型中的 Q/DQ 与张量 dtype 推断
- 权重存储: 保持 ONNX 原始 dtype（`float/int8/int16/int32/int64`）

## 快速结论

- C 后端覆盖率：`162/162`（100.0%）
- 量化覆盖率（按 `quant_ops`）：`162/162`（100.0%）
- 受约束算子：`22` 个，建议优先评审该列表
