...
@register_op("Div")
def emit_div(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) != 2:
        raise ValueError("Div expects 2 inputs.")
    ...
    if out_dtype in ("int8", "int16"):
        if out_shape != a_shape or out_shape != b_shape:
            raise ValueError("Quantized Div requires equal shapes.")
        if ctx.dtype(node.inputs[0]) != out_dtype or ctx.dtype(node.inputs[1]) != out_dtype:
            raise ValueError("Quantized Div requires matching dtypes.")
    ...
    if out_dtype != "float32":
        raise ValueError("Div supports float32 or quantized int8/int16 only.")
