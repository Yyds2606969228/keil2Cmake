...
@register_op("Ceil")
def emit_ceil(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) != 1:
        raise ValueError("Ceil expects 1 input.")
    ...
    if out_dtype in ("int8", "int16"):
        if ctx.dtype(node.inputs[0]) != out_dtype:
            raise ValueError("Quantized Ceil requires matching dtypes.")
        ...
        if so_zo is None:
            ...
            so, zo = so_zo
        emit_op_unary_quant(ctx.lines, out, inp, size, "ceilf(r)", out_dtype, sa, za, so, zo)
        return
    if out_dtype != "float32":
        raise ValueError("Ceil supports float32 or quantized int8/int16 only.")
    emit_op_unary_func(ctx.lines, out, inp, size, "ceilf")
