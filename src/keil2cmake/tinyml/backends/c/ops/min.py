...
@register_op("Min")
def emit_min(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) != 2:
        raise ValueError("Min expects 2 inputs.")
    ...
    if out_dtype in ("int8", "int16"):
        if out_shape != a_shape or out_shape != b_shape:
            raise ValueError("Quantized Min requires equal shapes.")
        if ctx.dtype(node.inputs[0]) != out_dtype or ctx.dtype(node.inputs[1]) != out_dtype:
            raise ValueError("Quantized Min requires matching dtypes.")
        sa, za = ctx.qparams(node.inputs[0])
        sb, zb = ctx.qparams(node.inputs[1])
        so_zo = ctx.qparams_optional(out_tensor)
        if so_zo is None:
            if sa != sb or za != zb:
                raise ValueError("Quantized Min requires output qparams when inputs differ.")
            so, zo = sa, za
        else:
            so, zo = so_zo
    ...
    if out_dtype != "float32":
        raise ValueError("Min supports float32 or quantized int8/int16 only.")
    emit_op_binary_broadcast_func(ctx.lines, out, a, b, out_shape, a_shape, b_shape, "fminf")
