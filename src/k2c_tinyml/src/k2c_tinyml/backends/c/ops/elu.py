...
@register_op("Elu")
def emit_elu(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) != 1:
        raise ValueError("Elu expects 1 input.")
    ...
    if out_dtype in ("int8", "int16"):
        if ctx.dtype(node.inputs[0]) != out_dtype:
            raise ValueError("Quantized Elu requires matching dtypes.")
        ...
        if so_zo is None:
            ...
            so, zo = so_zo
        expr = f"(r >= 0.0f ? r : ({alpha:.8f}f * (expf(r) - 1.0f)))"
        emit_op_unary_quant(ctx.lines, out, inp, size, expr, out_dtype, sa, za, so, zo)
        return

    if out_dtype != "float32":
        raise ValueError("Elu supports float32 or quantized int8/int16 only.")
    ctx.lines.append(f"  for (size_t i = 0; i < {size}; ++i) {{")
    ctx.lines.append(f"    float v = {inp}[i];")
    ctx.lines.append(f"    {out}[i] = v >= 0.0f ? v : ({alpha:.8f}f * (expf(v) - 1.0f));")
    ctx.lines.append("  }")
