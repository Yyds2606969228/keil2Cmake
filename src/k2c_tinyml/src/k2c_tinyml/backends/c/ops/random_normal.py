...
def emit_random_normal(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) != 0:
        raise ValueError("RandomNormal expects 0 inputs.")
    if len(node.outputs) != 1:
        raise ValueError("RandomNormal expects 1 output.")

    out_name = node.outputs[0]
    out_dtype = ctx.dtype(out_name)
    if out_dtype not in ("float32", "int8", "int16"):
        raise ValueError("RandomNormal supports float32/int8/int16 output only.")

    out_shape = [int(v) for v in ctx.shape(out_name)]
    if any(v <= 0 for v in out_shape):
        raise ValueError("RandomNormal requires known positive output shape.")
    out_size = tensor_size(out_shape)
    mean = float(node.attrs.get("mean", 0.0))
    scale = float(node.attrs.get("scale", 1.0))
    if not scale > 0.0:
        raise ValueError("RandomNormal requires scale > 0.")
    seed = _seed_u32(node)
    ...
    state = ctx.next_symbol("k2c_rn_state")
