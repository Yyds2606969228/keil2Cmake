...
def emit_det(ctx: EmitContext, node: NodeInfo) -> None:
    ...
    batch = product(x_shape[:-2]) if len(x_shape) > 2 else 1
    expected_out = x_shape[:-2]
    if out_shape != expected_out:
        raise ValueError("Det output shape mismatch.")
    if tensor_size(out_shape) != batch:
        raise ValueError("Det output size mismatch.")
    ...
    ctx.lines.append(f"    size_t base = b * (size_t)({n} * {n});")
    if quant_in:
        ctx.lines.append(
            f"    for (size_t i = 0; i < {n} * {n}; ++i) {{ {mat_sym}[i] = ((float){x}[base + i] - {zx}) * {sx:.8f}f; }}"
        )
    else:
        ctx.lines.append(f"    for (size_t i = 0; i < {n} * {n}; ++i) {{ {mat_sym}[i] = {x}[base + i]; }}")
    ...
    ctx.lines.append("      }")
    ctx.lines.append("      if (max_abs <= 1e-12f) { det_v = 0.0f; break; }")
    ...
    if quant_out:
        ...
        ctx.lines.append(f"    {out}[b] = ({qctype})q;")
    else:
        ctx.lines.append(f"    {out}[b] = det_v;")
    ctx.lines.append("  }")
