...
def emit_random_uniform_like(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) != 1:
        raise ValueError("RandomUniformLike expects 1 input.")
    if len(node.outputs) != 1:
        raise ValueError("RandomUniformLike expects 1 output.")
    ...
    if in_shape != out_shape:
        raise ValueError("RandomUniformLike output shape must match input shape.")
    if any(v <= 0 for v in out_shape):
        raise ValueError("RandomUniformLike requires known positive shape.")

    out_dtype = ctx.dtype(out_name)
    if out_dtype not in ("float32", "int8", "int16"):
        raise ValueError("RandomUniformLike supports float32/int8/int16 output only.")

    out_size = tensor_size(out_shape)
    low = float(node.attrs.get("low", 0.0))
    high = float(node.attrs.get("high", 1.0))
    if not high > low:
        raise ValueError("RandomUniformLike requires high > low.")
    seed = _seed_u32(node)
    ...
    state = ctx.next_symbol("k2c_rul_state")
