...
@register_op("Mean")
def emit_mean(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) < 1:
        raise ValueError("Mean expects at least 1 input.")
    ...
    if out_dtype in ("int8", "int16"):
        for name in node.inputs:
            if ctx.dtype(name) != out_dtype:
                raise ValueError("Quantized Mean requires matching input/output dtypes.")
            if ctx.shape(name) != out_shape:
                raise ValueError("Quantized Mean requires equal input/output shapes.")
        ...
        ctx.lines.append(f"    ro = ro / (float){n_inputs};")
    ...
    if out_dtype != "float32":
        raise ValueError("Mean supports float32 or quantized int8/int16 only.")
    for name in node.inputs:
        if ctx.dtype(name) != "float32":
            raise ValueError("Float Mean requires float32 inputs.")
        if ctx.shape(name) != out_shape:
            raise ValueError("Float Mean currently requires equal input/output shapes.")
    ctx.lines.append(f"  for (size_t i = 0; i < {out_size}; ++i) {{")
    ctx.lines.append("    float acc = 0.0f;")
    for name in node.inputs:
        inp = ctx.map_ptr(name)
        ctx.lines.append(f"    acc += {inp}[i];")
    ctx.lines.append(f"    {out_ptr}[i] = acc / (float){n_inputs};")
    ctx.lines.append("  }")
