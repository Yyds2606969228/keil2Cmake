...
@register_op("Neg")
def emit_neg(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) != 1:
        raise ValueError("Neg expects 1 input.")
    ...
    if out_dtype in ("int8", "int16"):
        if ctx.dtype(node.inputs[0]) != out_dtype:
            raise ValueError("Quantized Neg requires matching dtypes.")
    ...
    if out_dtype != "float32":
        raise ValueError("Neg supports float32 or quantized int8/int16 only.")
    ctx.lines.append(f"  for (size_t i = 0; i < {size}; ++i) {{")
    ctx.lines.append(f"    {out}[i] = -{inp}[i];")
    ctx.lines.append("  }")
