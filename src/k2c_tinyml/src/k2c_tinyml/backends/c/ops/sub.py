...
from ....operators.utils import emit_op_binary_broadcast, tensor_size


@register_op("Sub")
def emit_sub(ctx: EmitContext, node: NodeInfo) -> None:
    if len(node.inputs) != 2:
        raise ValueError("Sub expects 2 inputs.")
    ...
    if out_dtype in ("int8", "int16"):
        if out_shape != a_shape or out_shape != b_shape:
            raise ValueError("Quantized Sub requires equal shapes.")
        if ctx.dtype(node.inputs[0]) != out_dtype or ctx.dtype(node.inputs[1]) != out_dtype:
            raise ValueError("Quantized Sub requires matching dtypes.")
        ...
        ctx.lines.append("    float ro = ra - rb;")
    ...
    if out_dtype != "float32":
        raise ValueError("Sub supports float32 or quantized int8/int16 only.")
    emit_op_binary_broadcast(ctx.lines, out, a, b, out_shape, a_shape, b_shape, "-")
